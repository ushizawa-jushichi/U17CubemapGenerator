			...
			public override IEnumerator SaveAsPNGCoroutine(string assetPath)
			{
...
#if USING_HDRP
				if (generator._exposureOverride && (generator._pipelineType == RenderPipelineUtility.PipelineType.HDPipeline))
				{
					generator.StartExposureOverride(generator._fixedExposure, generator._compensation);
				}
#endif

				if (generator._pipelineType == RenderPipelineUtility.PipelineType.BuiltInPipeline
#if USING_HDRP 
					|| generator._pipelineType == RenderPipelineUtility.PipelineType.HDPipeline
#endif
					)
				{
				...
				else
				{
					...
					_renderPipelineHook = new RenderPipelineHook(
						...
						onBeginContextRendering: (context, cameras) => OnBeginContextRendering(context, cameras)
#else
...
#if USING_HDRP
				if (generator._pipelineType == RenderPipelineUtility.PipelineType.HDPipeline)
				{
				}
				else
#endif
#if USING_URP
				if (generator._pipelineType == RenderPipelineUtility.PipelineType.UniversalPipeline)
				{
#if UNITY_2023_3_OR_NEWER
					UniversalRenderPipeline.SubmitRenderRequest(camera, context);
#else
